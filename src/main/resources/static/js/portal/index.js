...
$(document).ready(function() {
	...
	if(loginUser == undefined || loginUser == null){
	    ...
	    $("#topSideUserName")[0].innerText = loginUser.realname;
	}
	//获取菜单
	loadMenus();
...
function aClick(navid){
    var activelink = $("#mainnav-menu").find('a#menu_' + navid).parent('li');
    if (!!activelink) {
        var sibs = $("#mainnav-menu").find("li");
        if(sibs != null && sibs.length > 0){
        	$.each(sibs, function (index, sibNodes) {  
        		sibNodes.className = '';
        	});
        }
        activelink.addClass('active-link');
        activelink.parent().parent().addClass('active-link');
        
       $(window.frames["_iframe"].document).find("#firstMenuVal").text(activelink.parent().parent().find(".menu-title").text());
       $(window.frames["_iframe"].document).find("#secondMenuVal").text(activelink.text());
    }
}
