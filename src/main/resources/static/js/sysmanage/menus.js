...
//            if (field == 'sort') {	                 
//                $('#TableTree').ExtendSort({ idField: 'menuid', sortField: 'sort', tableName: 'base_menus', cellObj: $this, id: row.menuid });
//            }
//        }
    });
}
...
function menuModal(optype){
	...
	if(optype == 1){
		...
		if(selMenus == null){
	        ...
	        var checkableNodes = $("#parentmenuid_seltree").treeview('search', [ String(selMenus.parentmenuid), { ignoreCase: false, exactMatch: true } ]);
	        $("#parentmenuid_seltree").treeview('selectNode', [ checkableNodes, { silent: true }]);
	        var parentMenuName = "--请选择--";
...
//表单验证
function validForm(){
	var validators="validators: {notEmpty: {message: '菜单名称不能为空.'},stringLength: {min: 1,max: 20,message: '菜单名称长度必须在1~20位之间'}}"
